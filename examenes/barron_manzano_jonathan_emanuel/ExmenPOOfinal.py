...
def obtener_libros():
    try:
        libros_df = pd.read_csv("libros.txt", sep=';', names=['codigo', 'titulo', 'autor', 'localizacion', 'genero', 'disponible'])
...
def obtener_usuarios():
    try:
        usuarios_df = pd.read_csv("usuarios.txt", sep=';', names=['numero', 'nombre', 'direccion'])
...
def obtener_prestamos():
    try:
        prestamos_df = pd.read_csv("prestamos.txt", sep=';', names=['codigoLibro', 'numeroUsuario', 'fecha'])
        prestamos = [
            Prestamo(row['codigoLibro'], row['numeroUsuario'], datetime.strptime(row['fecha'], '%Y-%m-%d %H:%M:%S')) for
            _, row in prestamos_df.iterrows()]
        return prestamos
    except FileNotFoundError:
        return []
...
while True:
    ...
    if eleccion == "1":
        ...
        registrar_libro(Libro(codigo, titulo, autor, localizacion, genero, disponible.lower() == 'true'))
